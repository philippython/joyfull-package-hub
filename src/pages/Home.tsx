...
function NavCard({ to, eyebrow, title }: { to: string; eyebrow: string; title: string }) {
  return (
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative h-screen min-h-[560px] flex items-center justify-center overflow-hidden bg-[color:var(--color-black,#0d0a08)]">
        <img src={heroImg} alt="A couple sharing a quiet moment with the Rewindd ritual kit" className="absolute inset-0 w-full h-full object-cover opacity-60" />
        <div className="absolute inset-0 hero-overlay" />
        <div className="relative z-10 max-w-4xl text-center px-6">
          <div className="eyebrow mb-6 justify-center !text-[color:var(--color-gold)]">Date Night Ritual Kit</div>
          <h1 className="hero-headline">We didn't fall out of love.<br />Life just got busy.</h1>
          <p className="hero-subhead">For couples who still love each other but haven't had quality time together in a while.</p>
          <div className="mt-6 flex flex-col items-center gap-2">
            <Link to="/kit" className="btn-primary px-8 py-3 text-[13px] tracking-[0.22em]">GET YOUR DATE NIGHT KIT</Link>
      ...
      <section className="grid md:grid-cols-2 gap-16 md:gap-24 max-w-6xl mx-auto px-6 py-24 items-center">
        <div><img src={boxImg} alt="The Rewindd box, sealed and ready" loading="lazy" className="w-full aspect-[4/5] object-cover" /></div>
        <div>
          <div className="eyebrow mb-6">The Quiet Truth</div>
          <h2 className="section-title">You still love each other.<br /><em>Life has just been busy.</em></h2>
          <div className="mt-8 space-y-4 text-charcoal/80 leading-8">
            ...
            <p className="font-serif italic text-2xl text-charcoal leading-snug pt-2">Rewindd was created for couples who still love each other but want to be more intentional about spending quality time together.</p>
      ...
      <section id="product" className="bg-[color:var(--color-burgundy-deep)] text-cream px-6 py-24">
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <h2 className="section-title !text-cream mt-4">One Kit.<br /><em>More Time Together.</em></h2>
          </div>
          ...
          <div className="text-center mt-12"><Link to="/kit" className="btn-outline !text-gold !border-gold hover:!bg-gold hover:!text-burgundy-deep">Order your date night kit</Link></div>
        </div>
      </section>
        ...
        <div className="mt-16 max-w-6xl mx-auto grid md:grid-cols-3 gap-10">
          {steps.map((s) => (
            <div key={s.n} className="text-center">
              <div className="aspect-[4/5] overflow-hidden mb-6"><img src={s.img} alt={s.t} loading="lazy" className="w-full h-full object-cover" /></div>
              ...
              <div className="space-y-1 text-cream/80 text-[14px] leading-relaxed">{s.lines.map((l, i) => <p key={i}>{l}</p>)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* QUOTE PHOTO */}
      <section className="relative w-full overflow-hidden" style={{ height: "clamp(420px, 70vh, 760px)" }}>
        <img src={coupleCards} alt="A couple drawing cards together by candlelight" loading="lazy" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 20%", display: "block" }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0.25) 0%, rgba(0,0,0,0.05) 40%, rgba(0,0,0,0.55) 80%, rgba(0,0,0,0.85) 100%)" }} />
        <div style={{ position: "absolute", bottom: "8%", left: 0, right: 0, textAlign: "center", padding: "0 1.5rem", display: "flex", flexDirection: "column", alignItems: "center", gap: "1.5rem" }}>
          <p className="font-serif" style={{ fontStyle: "italic", fontSize: "clamp(1.4rem, 3.5vw, 3rem)", color: "#fff", lineHeight: 1.3, maxWidth: "36rem", margin: 0, textShadow: "0 2px 24px rgba(0,0,0,0.6)" }}>"One evening. No distractions. Just us."</p>
          <Link to="/kit" className="btn-primary" style={{ fontSize: "0.7rem", letterSpacing: "0.2em" }}>GET YOUR KIT — £39.99</Link>
        </div>
      </section>
      ...
      <section className="relative py-32 px-6 text-center text-cream overflow-hidden bg-charcoal">
        <img src={detail4} alt="" loading="lazy" className="absolute inset-0 w-full h-full object-cover opacity-30" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-black/70" />
        <div className="relative z-10 max-w-2xl mx-auto">
          <div className="eyebrow justify-center mb-6">Tonight, Not Someday</div>
          <h2 className="font-serif font-light text-[clamp(40px,6vw,72px)] leading-[1.1] text-cream">The love is still there.<br /><em>Come find it.</em></h2>
