...
export function SiteNav() {
  ...
  return (
    <nav
      ...
      style={{ background: scrolled ? "rgba(13,10,8,0.95)" : "rgba(13,10,8,0.75)", borderColor: "rgba(200,168,107,0.18)" }}
    >
      <div className="flex items-center justify-between px-6 md:px-10 py-4">
        <Link to="/" className="font-serif italic text-2xl tracking-wide text-gold" onClick={() => setOpen(false)}>Rewindd</Link>
        <ul className="hidden md:flex items-center gap-7 list-none">
          {NAV.map((item) => (
            <li key={item.to}>
              <Link to={item.to} className="text-[11px] tracking-[0.18em] uppercase text-cream/75 hover:text-gold transition">{item.label}</Link>
            </li>
          ))}
          <li><Link to="/kit" className="btn-primary !py-2.5">Get the Rewindd Kit</Link></li>
        </ul>
        <button aria-label="Toggle menu" onClick={() => setOpen((v) => !v)} className="md:hidden flex flex-col gap-[5px] p-2">
      ...
      {open && (
        <div className="md:hidden border-t border-gold/15 bg-[rgba(13,10,8,0.98)] px-6 py-6">
          <ul className="flex flex-col gap-4 list-none">
            {NAV.map((item) => (
              <li key={item.to}>
                <Link to={item.to} onClick={() => setOpen(false)} className="block text-[12px] tracking-[0.18em] uppercase text-cream/85 hover:text-gold transition py-1">{item.label}</Link>
              </li>
            ))}
            <li className="pt-2"><Link to="/kit" onClick={() => setOpen(false)} className="btn-primary w-full text-center !block">Get the Rewindd Kit</Link></li>
          </ul>
        </div>
      )}
    </nav>
    ...
    <footer className="bg-[color:var(--color-charcoal)] text-cream/70 px-6 md:px-12 pt-20 pb-8">
      <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-10 mb-12">
        <div>
          ...
          <p className="text-[13px] leading-relaxed text-cream/55 max-w-xs">For couples who still love each other, but miss spending quality time together.</p>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-[13px] list-none">
            ...
            <li><Link to="/kit" className="hover:text-gold transition">Rewindd Kits</Link></li>
        ...
        <div>
          ...
          <p className="text-[13px] text-cream/55 leading-relaxed"><a href="mailto:rewindd2026@gmail.com" className="hover:text-gold transition">rewindd2026@gmail.com</a></p>
        </div>
      </div>
