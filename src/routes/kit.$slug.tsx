...
export const Route = createFileRoute("/kit/$slug")({
  head: ({ params }) => ({
    meta: [
      ...
      { name: "description", content: "A romantic at-home date night kit for busy couples in the UK. Hand-packed, UK delivery, dispatched in 2–3 business days." },
    ],
  }),
  component: KitDetail,
});
...
function KitDetail() {
  ...
  const productImages = product.image_url ? [product.image_url, ...galleryImages] : galleryImages;
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <p className="text-[11px] text-warm-gray text-center">
        UK delivery only · Dispatched in 2–3 business days · Payment instructions will be emailed (Stripe checkout coming soon).
      </p>
    </form>
  );
}
